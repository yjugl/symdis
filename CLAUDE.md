...
- `symbols/pdb.rs` — PDB → `SymFile` converter via the `pdb` crate; extracts procedures, public symbols, and line programs per module; converts PDB section:offset addresses to RVAs via address map; zero-size procedures become PUBLIC symbols; `catch_unwind` around each module to survive `pdb` crate panics (silent panic hook suppresses output)
