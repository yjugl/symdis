...
- `disasm/annotate.rs` — Annotation pipeline: source lines, call target resolution (FUNC/PUBLIC/IAT/PLT + indirect via IAT/pointer tables for both x86-64 RIP-relative and x86 32-bit absolute addressing), inline frame tracking, highlight with mid-instruction range matching
