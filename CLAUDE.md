...
6. Annotate: source lines → call targets (direct + RIP-relative indirect) → inlines → highlight (`disasm/annotate.rs`)
...
- `disasm/engine.rs` — Capstone wrapper supporting x86/x86_64/ARM/ARM64 with Intel or ATT syntax; extracts call targets from direct call/jmp instructions; computes RIP-relative memory slot RVAs for indirect call/jmp via x86 detail API
- `disasm/annotate.rs` — Annotation pipeline: source lines, call target resolution (FUNC/PUBLIC/IAT/PLT + RIP-relative indirect via IAT/pointer tables), inline frame tracking, highlight with mid-instruction range matching
