...
- `binary/elf.rs` — Goblin-based ELF parser implementing `BinaryFile` trait; VA-to-offset via PT_LOAD segments; PLT import mapping for call target annotation with architecture-specific header sizes (AArch64: 32+16, ARM: 20+12, x86/x86_64: 16+16); searches `.plt`, `.plt.got`, and `.iplt` sections
...
- `disasm/engine.rs` — Capstone wrapper supporting x86/x86_64/ARM/ARM64 with Intel or ATT syntax; extracts call targets from direct call/jmp instructions; computes memory slot RVAs for indirect call/jmp (x86-64 RIP-relative and x86 32-bit absolute addressing via x86 detail API)
