...
- `fetch/apt.rs` — Ubuntu APT package client; downloads Packages.xz index from `archive.ubuntu.com` (or `ports.ubuntu.com` for arm64/armhf); searches base release, `-updates`, and `-security` pockets; resolves binary packages by Package: or Source: field; downloads `.deb` files (ar archives); extracts binaries from `data.tar.{zst,xz,gz}` via zstd/liblzma/flate2+tar; handles symlinks in tar (follows `libfoo.so.0` → `libfoo.so.0.8000.0`); source package name auto-detected from `.sym` source paths (`/build/<name>-<random>/<name>-<version>/...` or `/usr/src/<name>-<version>/...` patterns, distinct from snap paths)
