...
- `binary/pe.rs` — Goblin-based PE parser implementing `BinaryFile` trait; RVA-to-file-offset via section table walk; IAT import resolution for call target annotation; `.pdata` exception entries parsed for exact function bounds (`function_bounds()` trait method)
